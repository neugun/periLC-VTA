...
zi_nb_glmm = update(
  nb_glmm, 
  ziformula=~time_bin,
# In case of model convergence problems, use a more robust optimizer:
...
# DATASET-SPECIFIC ASSESSMENT of ZI-NB GLMM: 
# - overdispersion ratio is ~1
# - zeros are better predicted
# - use ZI-NB GLMM


# Manually enter the final model you want to use for statistical analysis. 
final_model = zi_nb_glmm # <---------- ENTER FINAL MODEL HERE
...
no_laser_model = update(
  ...
  control = glmmTMBControl(
  optimizer = optim,
  optArgs = list(method = "Nelder-Mead"), # "BFGS" and "Nelder-Mead"
  optCtrl = list(maxit = 1000) # Increase the number of iterations
  )
)
summary(no_laser_model)
```
